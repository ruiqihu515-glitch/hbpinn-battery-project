...
def main():
    ...
    prediction_path = output_dir / f"temperature_aware_hybrid_voltage_predictions_{battery_id}.csv"
    ...
    metrics_path = output_dir / f"temperature_aware_hybrid_voltage_metrics_{battery_id}.csv"
    ...
    figure_path = (
        figure_dir
        / f"temperature_aware_hybrid_voltage_prediction_{battery_id}_cycle_{plot_cycle}.png"
    )
    ...
    plt.plot(
        ...
        linewidth=2,
        label="Temperature-aware hybrid prediction",
    )

    plt.xlabel("Time [s]")
    plt.ylabel("Voltage [V]")
    plt.title(f"{battery_id} temperature-aware hybrid voltage prediction, cycle {plot_cycle}")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    ...
    per_cycle_metrics_path = (
        output_dir / f"temperature_aware_hybrid_voltage_per_cycle_metrics_{battery_id}.csv"
    )
